...
This file is the source of truth for the manually maintained Chrome Web Store and Microsoft Edge Add-ons listings, privacy declarations, and certification notes.
...
The extension locally processes the configured email or UPN, Microsoft sign-in URL context, and Microsoft account-picker page content. Source settings and sanitized diagnostics are stored in the browser profile's local extension storage. While URL rewriting is configured, the active account/domain hints and application matches are also represented in browser-managed dynamic rules. When a supported sign-in without an application-provided hint is visited, the configured account or domain hint is sent directly to Microsoft's sign-in service as part of that request. No extension data is sent to a developer server, sold, or used for advertising or analytics.
...
- Store icon: `docs/images/store-icon-300.png` (300 x 300; use the packaged 128 x 128 icon only where a store explicitly requests it)
- Screenshot 1: `docs/images/store-screenshot-01-popup.png` (1280 x 800)
...
All listing images are RGB PNGs without transparency and are regenerated by `node scripts/qa-loaded-extension.mjs` from the verified extension build.
...
- Store icon, minimum-size fallback: `release/webstore-assets/store-icon-128.png`
- Store icon, preferred: `release/webstore-assets/store-icon-300.png`
